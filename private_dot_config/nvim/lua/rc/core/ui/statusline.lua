...
local function file_size(buffer)
  ...
  local ret = ""
  if size < kb then
    ret = string.format("%.1f%s", size, "B")
  elseif size < mb then
    ret = string.format("%.1f%s", size / kb, "KiB")
  elseif size < gb then
    ret = string.format("%.1f%s", size / mb, "MiB")
  elseif size < tb then
    ret = string.format("%.1f%s", size / gb, "GiB")
  else
    ret = string.format("%.1f%s", size / tb, "TiB")
  end
  return ret
end
