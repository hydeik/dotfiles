...
function setupLsp(args: ConfigArguments) {
	...
	for (const scope of ["document", "workspace"]) {
	...
	for (const method of ["incomingCalls", "outgoingCalls"]) {
		args.contextBuilder.patchLocal(`lsp_${method}`, {
			sources: [
				{
	...
	for (const method of ["supertypes", "subtypes"]) {
		args.contextBuilder.patchLocal(`lsp_${method}`, {
			sources: [
				{
			...
			dpp: {
				...
				actions: {
					update: async (args: ActionArguments<Params>) => {
						const names = args.items.map(
							(item) => (item.action as DppAction).__name,
						);
