import { ActionFlags, type DduOptions } from "jsr:@shougo/ddu-vim@~6.4.0/types";
...
async function setUiSize(args: ConfigArguments) {
	...
	args.contextBuilder.patchGlobal({
		uiParams: {
			ff: {
				...(await calculateUiSize(args.denops, previewRatio, mayVSplit)),
			} as Partial<FfParams>,
		},
	});
	...
	args.contextBuilder.patchLocal("search", {
		uiParams: {
			ff: {
				...(await calculateUiSize(args.denops, previewRatio, false)),
			} as Partial<FfParams>,
		},
	});
}
				...
				ff: {
					actions: {
						kensaku: async (args: {
							...
							await args.denops.dispatcher.updateOptions(args.options.name, {
								sourceOptions: {
									_: {
